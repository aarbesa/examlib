...
void exam::ask_param(void)
{
    ...
    connexion();
    set_max_time();
    std::cout << WHITE << BOLD << "Press a key to start exam my dear" << LIME << BOLD  << " Arbesa <3 " << RESET << std::endl;
    if (!std::getline(std::cin, enter))
        sigd();
    // ====================
...
exam::exam(void) : exam_grade(0), level(2), level_max(5), failures(0), student(false), backup(false), using_cheatcode(1)
{
    reelmode = true;
    waiting_time = true;
	vip = 0;
